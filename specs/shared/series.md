...
The Series Card is a reusable component throughout GCashTV that displays information about a series. It appears in carousels and grids on the Home page, Player page recommendations, and Genre page.

This specification explains the Series Card for everyone, including designers, product managers, QA, and engineers.

---

## Layout

Series Cards are organized vertically:
...
- Width: fit-content (as wide as the text)
- Background: #2A2A2E
- Color: #9CA3AF
- Font Size: 12px
- Padding: 4px 10px
...
- Background: #3B82F6
- Color: #FFFFFF
- Title Override: When tag is hovered, title remains white
  - CSS: `.series-card:has(.series-tag:hover) .series-title { color: #FFFFFF; }`

### Click Action
- Navigate to `/genre?category={tagName}`
- Prevent card click from triggering using `e.stopPropagation()`
...
A horizontal scrolling carousel section used to display Series Cards. Appears in "You Might Like" and "New Releases" sections on Home and Player pages.
...
- Scroll Amount: 80% of container width per click
- Direction: Left or Right based on button
- Behavior: Smooth scrolling using scrollBy()
...
- Transition: color 0.2s ease
- Hover: #3B82F6
- SVG: Chevron right icon (scales dynamically)
...
## Overview
A recommendation carousel showing series the user may enjoy based on viewing history or preferences.

## Implementation
- Uses Series Carousel Section component
- Title: "You Might Like" (i18n key: `t.home.youMightLike`)
- Data Source: `youMightLikeSeries` array
- Filtering: Excludes current series on Player page
...
## Overview
A carousel section showing newly released series content.

## Implementation
- Uses Series Carousel Section component
- Title: "New Releases" (i18n key: `t.home.newReleases`)
- Data Source: `newReleasesSeries` array
- Filtering: Excludes current series on Player page
